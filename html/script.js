...
let matchingPositions = [0, 0, 0, 0];
const open = new Audio('sounds/wiringfix_open.mp3');
const close = new Audio('sounds/wiringfix_close.mp3');
const complete = new Audio('sounds/wiringfix_complete.mp3');
const connect = [new Audio('sounds/wiringfix_one.mp3'), new Audio('sounds/wiringfix_two.mp3'), new Audio('sounds/wiringfix_three.mp3'), new Audio('sounds/wiringfix_four.mp3')];
...
function toggleUI(bool, timer) {
    if (bool) {
      ...
      timerInterval = null;
        container.innerHTML = `<svg width="907" height="907" viewBox="0 0 907 907">
        ...
        <g style="isolation:isolate">
          ...
          <path class="od" d="M794.5 169.5h108v33h-108z"/>
          <path class="pd" d="M794.5 178.5h108v17h-108z"/>
          ...
          <path class="sd" d="M793.5 726.5h108v33h-108z"/>
          <path class="td" d="M793.5 735.5h108v17h-108z"/>
          <path class="qd" d="M791.5 541.5h110v33h-110z"/>
          <path class="rd" d="M791.5 550.5h110v17h-110z"/>
          <path class="md" d="M791.5 357.5h111v33h-111z"/>
          <path class="nd" d="M791.5 366.5h111v17h-111z"/>
          <path class="m" d="M8.5 167.5h66v33h-66z"/>
          <path class="n" d="M8.5 176.5h66v17h-66z"/>
          <g>
      ...
      `
        ...
        // Update the destination colors
        // 0
        color = getColor(0);
        document.getElementsByClassName('pd')[0].style.fill = color[0];
        document.getElementsByClassName('od')[0].style.fill = color[1];

        // 1
        color = getColor(1);
        document.getElementsByClassName('nd')[0].style.fill = color[0];
        document.getElementsByClassName('md')[0].style.fill = color[1];

        // 2
        color = getColor(2);
        document.getElementsByClassName('rd')[0].style.fill = color[0];
        document.getElementsByClassName('qd')[0].style.fill = color[1];

        // 3
        color = getColor(3);
        document.getElementsByClassName('td')[0].style.fill = color[0];
        document.getElementsByClassName('sd')[0].style.fill = color[1];
        ...
        onDrag: function () { updateLine('.line-1', this.x + 120, this.y + 185); },
            onRelease: function () {
                if (this.x !== 670 || this.y !== (matchingPositions[0]-0)*188 ) {
                reset('.drag-1', '.line-1', 70, 185);
                toggleLight(2, false);
                } else if (this.x === 670 && this.y === (matchingPositions[0]-0)*188) toggleLight(matchingPositions[0]+1, true)
            },
            liveSnap: {points: [{x: 670, y: (matchingPositions[0]-0)*188}],radius: 20}
        });
        new Draggable('.drag-2', {
            onDrag: function () { updateLine('.line-2', this.x + 120, this.y + 375); },
            onRelease: function () {
                if (this.x !== 670 || this.y !== (matchingPositions[1]-1)*188 ) {
                reset('.drag-2', '.line-2', 60, 375);
                toggleLight(1, false);
                } else if (this.x === 670 && this.y === (matchingPositions[1]-1)*188) toggleLight(matchingPositions[1]+1, true)
            },
            liveSnap: {points: [{x: 670, y: (matchingPositions[1]-1)*188}],radius: 20}
        });
        new Draggable('.drag-3', {
            onDrag: function () { updateLine('.line-3', this.x + 120, this.y + 560); },
            onRelease: function () {
                if (this.x !== 670 || this.y !== (matchingPositions[2]-2)*188 ) {
                reset('.drag-3', '.line-3', 60, 560);
                toggleLight(3, false);
                } else if (this.x === 670 && this.y === (matchingPositions[2]-2)*188) toggleLight(matchingPositions[2]+1, true)
            },
            liveSnap: {points: [{x: 670, y: (matchingPositions[2]-2)*188}],radius: 20}
        });
        new Draggable('.drag-4', {
            onDrag: function () { updateLine('.line-4', this.x + 120, this.y + 745); },
            onRelease: function () {
                if (this.x !== 670 || this.y !== (matchingPositions[3]-3)*188 ) {
                reset('.drag-4', '.line-4', 60, 745);
                toggleLight(4, false);
                } else if (this.x === 670 && this.y === (matchingPositions[3]-3)*188) toggleLight(matchingPositions[3]+1, true)
            },
            liveSnap: {points: [{x: 670, y: (matchingPositions[3]-3)*188}],radius: 20}
        });
        timerSeconds = timer;
        open.play();
        startTimer();
      ...
      fetch(`https://${GetParentResourceName()}/wiringfixFail`, {
          method: "POST",
          headers: {
...
function getColor(pos) {
  ...
  switch (pos) {
    case 0:
      return ["#324d9c", "#25378d"];
    case 1:
      return ["#e52320", "#a71916"];
    case 2:
      return ["#ffeb13", "#aa9f17"];
    case 3:
      return ["#a6529a", "#90378c"];
  }
}
  ...
  if (visibility) {
    ...
    if (completedLights[0] === 1 && completedLights[1] === 1 && completedLights[2] === 1 && completedLights[3] === 1) {
      ...
      window.setTimeout(() => {
        ...
        timerInterval = null;
        container.style.display = 'none';
        fetch(`https://${GetParentResourceName()}/wiringfixSuccess`, {
            method: "POST",
